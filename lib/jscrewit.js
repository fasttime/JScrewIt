...
(function (self)
{
    ...
    function ScrewBuffer(strongBound, maxUngroupedTokens)
    {
        ...
        this.append =
            function (solution)
            {
                ...
                if (lastSolution)
                {
                    ...
                    if (lastSolution.level < LEVEL_OBJECT && solution.level < LEVEL_OBJECT)
                    {
                ...
                else
                {
                    ...
                    else if (tokenCount >= maxUngroupedTokens)
                    {
                        lastSolution = solution;
                        minOverhead = solution.length + (hasOuterPlus(solution) ? 3 : 1);
                    }
                    else
                    {
                ...
                return head.length + minOverhead + tail.length;
            };
        this.toString =
            function ()
            {
    ...
    Encoder.prototype =
    {
        ...
        {
            var MAX_CONCAT_TOKENS = 2048; // limit imposed by Android Browser on Intel Atom
            
            var buffer = new ScrewBuffer(strongBound, MAX_CONCAT_TOKENS - 1);
            if (!simplePattern)
            {
            ...
            while (match = regExp.exec(string))
            {
                ...
                if (token in SIMPLE)
                {
                    ...
                    if (level === undefined)
                    {
                        ...
                        solution.level = value === undefined ? LEVEL_UNDEFINED : LEVEL_NUMERIC;
                    }
                }
                else
                {
                ...
                var length = buffer.append(solution);
                if (length > maxLength)
                {
                    return;
                }
