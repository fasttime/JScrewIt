...
(function (self)
{
        ...
        else if (quotedString)
        {
            ...
            catch (e)
            {
                throw new SyntaxError(
                    'Illegal string ' + quotedString + ' in the definition of ' +
            ...
            replacement = this.resolveString(string, strongBound);
        }
        else if (space)
        {
            replacement = '';
    ...
    Encoder.prototype =
    {
        ...
        {
            var MIN_DICT_ENCODABLE_LENGTH = 100;
            ...
            if (input.length >= MIN_DICT_ENCODABLE_LENGTH)
            {
                ...
                var freqList =
                    Object.getOwnPropertyNames(freqs).map(
                        function (char) { return freqs[char]; }
                    ).sort(function (freq1, freq2) { return freq2.count - freq1.count; });
                var reindexMap = createReindexMap(freqList.length);
                freqList.forEach(function (freq, index) { freq.reindex = reindexMap[index]; });
                freqList.sort(function (freq1, freq2) { return freq1.reindex - freq2.reindex; });
                var dict = freqList.map(function (freq) { return freq.char; }).join('');
                var indices =
                    Array.prototype.map.call(
                        input,
                        function (char)
                        {
                            return freqs[char].reindex;
                        }
                    ).join(false);
                dictOutput =
                    this.resolveString(indices, true) + this.replace('["split"](false)["map"]') +
                    '(' + this.replace('""["charAt"]["bind"]') + '(' + this.encodeString(dict) +
                    '))' + this.replace('["join"]([])');
            }
            var regularOutput = this.encodeString(input);
            var output =
                dictOutput && dictOutput.length < regularOutput.length ? dictOutput : regularOutput;
            if (wrapWithEval)
            {
            ...
            if (inputLength >= MIN_SUBENCODABLE_LENGTH)
            {
                var charCodes =
                    this.resolveString(
                        Array.prototype.map.call(
                            input,
                            function (char) { return char.charCodeAt(); }
                        )
                        .join(false),
                        true
                    ) + this.replace('["split"](false)');
                if (inputLength > this.maxDecodableArgs)
                {
                    output =
                        charCodes + this.replace('["map"]') + '(' +
                        this.resolveConstant('Function') + '(' + this.resolveString('return ') +
                        '+' + this.resolveConstant('STRING') + '+' +
                        this.resolveString('.fromCharCode(arguments[0])') + '))' +
                        this.replace('["join"]([])');
                }
                else
                {
                    output =
                        this.resolveConstant('Function') + '(' + this.resolveString('return ') +
                        '+' + this.resolveConstant('STRING') + '+' +
                        this.resolveString('.fromCharCode(') + '+' +
                        charCodes + '+' + this.resolveString(')') + ')()';
                }
            }
            output = this.encodeStringPlain(input, output && output.length) || output;
            return output;
        },
        
        encodeStringPlain: function (input, maxLength)
        {
            var output = this.resolveString(input, false, maxLength);
            return output;
        },
        ...
        resolveString: function (string, strongBound, maxLength)
        {
    ...
    if (typeof DEBUG === 'undefined' || DEBUG)
    {
        (function ()
        {
            ...
            var encoderProto = Encoder.prototype;
            encoderProto.constructor = Encoder;
            Object.freeze(encoderProto);
