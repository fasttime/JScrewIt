...
(function (self)
{
    ...
    var FEATURE_INFOS =
    {
        ...
        DEPRECATED:
        {
            description:
                'Deprecated features.\n' +
                'Those features are not available in any engine, but they can be explicitly ' +
                'enabled where native support or emulation exists.',
            includes: ['QUOTE']
        }
    };
    ...
    function getAppendLength(solution)
    {
        var length = solution.length + (hasOuterPlus(solution) ? 3 : 1);
        return length;
    }
