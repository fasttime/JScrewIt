...
(function (self)
{
    ...
    function replaceToken(wholeMatch, number, quotedString, space, literal, offset, expr)
    {
        ...
        else if (literal)
        {
            ...
            {
                solution = this.resolveSimple(literal);
            }
            else
            {
                throw new SyntaxError(
        ...
        encode: function (input, wrapWithEval)
        {
            ...
            if (input.length >= MIN_DICT_ENCODABLE_LENGTH)
            {
                ...
                freqList.sort(function (freq1, freq2) { return freq2.count - freq1.count; })
                    .forEach(function (freq, index) { freq.reindex = reindexMap[index]; });
                var dict =
                    freqList.sort(function (freq1, freq2) { return freq1.reindex - freq2.reindex; })
                    .map(function (freq) { return freq.char; }).join('');
                var indices =
                ...
                dictOutput =
                    indices + this.replace('["map"]') + '(' + this.replace('""["charAt"]["bind"]') +
                    '(' + this.encodeString(dict) + '))' + this.replace('["join"]([])');
            }
            ...
            if (inputLength >= MIN_SUBENCODABLE_LENGTH)
            {
                ...
                else
                {
                    output =
                        this.resolveConstant('Function') + '(' + this.replaceString('return ') +
                        '+' + this.resolveConstant('STRING') + '+' +
                        this.replaceString('.fromCharCode(') + '+' +
                        charCodes + '+' + this.replaceString(')') + ')()';
                }
            }
