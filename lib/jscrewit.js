...
(function () {
    ...
    var RULES = [
        ...
        // NUMERIC, PREFIXED_STRING
        {
            match: [
                ...
                createMatcher(SolutionType.UNDEFINED, SolutionType.NUMERIC, SolutionType.PREFIXED_STRING),
            ],
            replace: function (r1, r2) { return "[" + r1 + "]+" + r2; },
            solutionType: SolutionType.STRING,
        },
    ...
    var WEAK_MATCHER = createMatcher(SolutionType.WEAK_NUMERIC, SolutionType.WEAK_PREFIXED_STRING);
    ...
    var testType = function (matcher, type) { return (matcher & type) !== 0; };
    ...
    var DynamicSolution = /** @class */ (function (_super) {
        ...
        Object.defineProperty(DynamicSolution.prototype, "source", {
            get: function () {
                var source = this._solutions.map(function (_a) {
    ...
    function calculateReplacement(solutions) {
        switch (solutions.length) {
            ...
            default:
                {
                    var _a = findRule(solutions), match = _a.match, replace = _a.replace;
                    var matcherCount = match.length;
                    var replacements = solutions.slice(0, matcherCount).map(getReplacement);
                    var replacement = replace.apply(void 0, replacements) +
                        solutions.slice(matcherCount).map(function (_a) {
                            var replacement = _a.replacement, type = _a.type;
                            return testType(WEAK_MATCHER, type) ? "+(" + replacement + ")" : "+" + replacement;
                        })
                            .join('');
                    return replacement;
                }
    ...
    function findRule(solutions) {
        var rule = RULES.find(function (_a) {
            var match = _a.match;
            return testMatch(match, solutions);
        });
        return rule;
    }
    ...
    function testMatch(matchers, solutions) {
        if (matchers.length > solutions.length)
            return false;
        var test = matchers.every(function (matcher, index) {
            var solution = solutions[index];
            var test = testType(matcher, solution.type);
            return test;
        });
        return test;
    }
    ...
    function Solution(source, replacement, level, hasOuterPlus)
    {
