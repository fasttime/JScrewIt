...
(function (self)
{
    ...
    Encoder.prototype =
    {
        ...
        encode: function (input, wrapWithEval)
        {
            ...
            var output1 =
                this.resolveConstant('Function') + '(' +
                this.resolveString('return String.fromCharCode(') + '+' +
                this.resolveString(enc, true) +
                this.replace('["split"](false)') + '+' +
                this.resolveString(')') + ')()';
            var output2 = this.resolveString(input, false, output1.length);
            var output = output2 == null ? output1 : output2;
            if (wrapWithEval)
            {
        ...
        resolveString: function (string, strongBound, maxLength)
        {
            ...
            if (string)
            {
                ...
                while (match = regExp.exec(string))
                {
                    ...
                    fullLevel = parts.length ? LEVEL_STRING : level;
                    parts.push(solution);
            ...
            fullLength += (partCount - 2) / (MAX_CONCAT - 1) << 1;
            var enclosure = partCount > 1 && strongBound;
            if (enclosure)
            {
                fullLength += 2;
            }
            ...
            while (parts.length > MAX_CONCAT)
            {
                parts.push('(' + parts.splice(-MAX_CONCAT).join('+') + ')');
            }
