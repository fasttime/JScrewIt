    ...
    Encoder.prototype =
    {
        ...
        encode: function (input, wrapWithEval)
        {
            ...
            if (inputLength >= MIN_SUBENCODABLE_LENGTH)
            {
                subencOutput =
                    this.resolveConstant('Function') + '(' +
                    this.resolveString('return ');
                for (var index = 0; index < inputLength; index += MAX_ARGS)
                {
                    if (index)
                    {
                        subencOutput += '+' + this.resolveString('+');
                    }
                    var subencoding =
                        Array.prototype.map.call(
                            input.substr(index, MAX_ARGS),
                    ...
                    subencOutput +=
                        ...
                        this.resolveString(subencoding, true) + this.replace('["split"](false)') +
                        '+' + this.resolveString(')');
                }
                subencOutput += ')()';
            }
