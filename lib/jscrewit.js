    ...
    var FEATURE_INFOS =
    {
        ...
        FILL:
        {
            description:
                ...
                'Available in Firefox, Microsoft Edge, and in Chrome 45, Opera 32, Safari 7.1, ' +
...
(function ()
{
    ...
    var encoderProtoSource =
    {
        ...
        createDictArray: function (array, mapper, legend)
        {
            var dictArray =
                array + this.replaceExpr('["map"]') + '(' + this.replaceExpr(mapper) + '(' +
                legend + '))';
            return dictArray;
        },
        
        createDictEncoding: function (legend, charIndexes, radix, amendings, coerceToInt)
        {
            var mapper;
            if (radix)
            {
            ...
            var output =
                this.createDictArray(charIndexes, mapper, legend) +
        ...
        encodeByDblDict: function (inputData, maxLength)
        {
            ...
            var intermediateMaxLength = maxLength - legend.length;
            var intermediateLegend =
                this.replaceFalseFreeArray(figures, intermediateMaxLength - minFreqIndexLength);
            if (!intermediateLegend)
            {
                return;
            }
            var intermediateArray =
                this.replaceFalseFreeArray(
                    intermediateList,
                    intermediateMaxLength - intermediateLegend.length
                );
        ...
        encodeByDict: function (inputData, radix, amendings, maxLength)
        {
            ...
            var charIndexes =
                this.replaceFalseFreeArray(intermediateList, maxLength - legend.length);
            if (!charIndexes)
            {
                return;
            }
            var output =
                this.createDictEncoding(legend, charIndexes, radix, amendings, coerceToInt);
            if (!(output.length > maxLength))
            {
                return output;
            }
