    ...
    Encoder.prototype =
    {
        ...
        encode: function (input, wrapWithEval)
        {
            ...
            if (inputLength >= MIN_SUBENCODABLE_LENGTH)
            {
                ...
                if (inputLength > MAX_ARGS)
                {
                    subencOutput =
                        subencoding +
                        this.replace(
                            '["split"](false)["map"](Function("return String.fromCharCode(argumen' +
                            'ts[0])"))["join"]([])'
                        );
                }
                else
                {
                    subencOutput =
                        ...
                        this.resolveString('.fromCharCode(') + '+' + subencoding +
                        this.replace('["split"](false)') + '+' + this.resolveString(')') + ')()';
                }
            }
            var output =
                this.resolveString(input, false, subencOutput && subencOutput.length) ||
                subencOutput;
            if (wrapWithEval)
            {
